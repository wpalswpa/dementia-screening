...
사전 준비:
  ...
  2) SQL Developer 등에서 database/01_create_centers_table.sql 실행 (테이블 생성)
  3) 아래 접속 정보를 자기 환경에 맞게 수정
...
DB_USER = "system"
DB_PASSWORD = "oracle"
DB_DSN = "localhost:1521/xe"   # 예: XE 기본 설치 기준
...
def main():
    ...
    for _, r in df.iterrows():
        rows.append((
            ...
            str(r.get("관리기관전화번호", ""))[:50],
            str(r.get("운영기관전화번호", ""))[:50],
            str(r.get("관할지자체", ""))[:100],
            str(r.get("주요프로그램", ""))[:2000],
            str(r.get("데이터기준일자", ""))[:20],
        ))
